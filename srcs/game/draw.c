...
/*   Updated: 2021/05/10 13:58:28 by ketaouki         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (s->raycast.side == 0)
		s->t.wall_x = s->player.position_y + s->raycast.wall_dist
			* s->raycast.raydir_y;
	else
		s->t.wall_x = s->player.position_x + s->raycast.wall_dist
			* s->raycast.raydir_x;
	...
	while (s->raycast.draw_start < s->raycast.draw_end)
	{
		...
		s->addr[s->raycast.draw_start * s->line_length / 4 + s->raycast.x]
			= s->t.addr[s->t.line_length / 4 * s->t.texture_y + s->t.texture_x];
