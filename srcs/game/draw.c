...
/*   Updated: 2021/05/11 13:48:20 by ketaouki         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_wall(t_cub *s)
{
	...
	s->te.texture_x = (int)(s->te.wall_x * s->t.texture_width);
	if (s->raycast.side == 0 && s->raycast.raydir_x > 0)
		s->te.texture_x = s->t.texture_width - s->te.texture_x - 1;
	if (s->raycast.side == 1 && s->raycast.raydir_y < 0)
		s->te.texture_x = s->t.texture_width - s->te.texture_x - 1;
	s->te.step = 1.0 * s->t.texture_height / s->raycast.line_height;
	s->te.texture_position
		= (s->raycast.draw_start - s->height / 2 + s->raycast.line_height / 2)
		* s->te.step;
			s->te.texture_x = s->t.line_length / 4 - 1 - s->te.texture_x;
	while (s->raycast.draw_start < s->raycast.draw_end)
	{
		s->te.texture_y = (int)s->te.texture_position & (s->t.texture_height - 1);
		s->te.texture_position += s->te.step;
		s->addr[s->raycast.draw_start * s->line_length / 4 + s->raycast.x]
			= s->t.addr[s->te.texture_y * (s->t.line_length / 4) + s->te.texture_x];
